...
Dim sS As String
    ...
    If Not MyRsORG.EOF Then 'Debe existir
        ...
        '    If TeclaTmp.Key <> MyRsDes.Fields("key") Then
        '        MsgBox "Código de Artículo ya asignado a tecla:" & CStr(MyRsDes.Fields("key")), vbCritical
        '        GoTo salir
        '    End If
        'End If
        Set MyRsDes = MyBaseDES.OpenRecordset("select * from items where key=" & CStr(TeclaTmp.Key))
        ...
        MyRsDes.Fields("name") = TeclaTmp.Name
        ...
        MyRsDes.Fields("family-codi_sub") = MyRsORG.Fields("codi_sub")
        ...
        End If
        MyRsDes.Fields("plu") = TeclaTmp.PLU
        MyRsDes.Fields("codi_ident") = MyRsORG.Fields("codi_ident")
        ...
        myRsTMP.Close
        Set myRsTMP = Nothing
        MyRsDes.Fields("vat_value") = 0 'buscar el porcentaje
        ...
        MyRsDes.Fields("label_text") = "label_alcampo.xml"
        sS = extraeBMP(TeclaTmp.icon_name)
        'If Mid(sS, 4) = "" Then
        '    sS = sS
        'End If
        MyRsDes.Fields("icon_name") = Mid(sS, 4)
        MyRsDes.Fields("icon_path") = App.Path & "\items\" 'Mid(TeclaTmp.icon_name, 1, Val(Mid(sS, 1, 3)))
        MyRsDes.Fields("grp") = TeclaTmp.Group
        MyRsDes.Fields("subgroup") = TeclaTmp.SubGroup
        MyRsDes.Fields("set") = 0
        MyRsDes.Fields("key") = TeclaTmp.Key
        If TeclaTmp.qLevelName <> "" Then
        sS = extraeBMP(TeclaTmp.qLevelName)
        MyRsDes.Fields("qualityname") = Mid(sS, 4)
        'localizar en la tabla quality
        Set myRsTMP = MyBaseDES.OpenRecordset("select * from qlevels where trim(icon)=" & "'" & Mid(sS, 4) & "'")
        If myRsTMP.EOF Then
            MyRsDes.Fields("qualitynr") = 0
        Else
            MyRsDes.Fields("qualitynr") = Val(myRsTMP.Fields("code"))
        End If
        myRsTMP.Close
        Set myRsTMP = Nothing
        Else
...
Dim n As Integer
    ...
    FrmKeyTouch.CmdEnvTodo.Visible = True
