    ...
    If (CuentaGRande >= 30 And TimeAuto <= 5 And (TimeAutoSeg = 0)) Or (TimeAuto > 5 And (CuentaGRande * 2) >= TimeAuto And (TimeAutoSeg = 0)) Or Trim(Command) = "/01PEEKTOT" Or ((CuentaGRande * 2) >= TimeAutoSeg And (TimeAutoSeg <> 0)) Then
        ...
        If (MyContador >= TimeAuto And TimeAuto <= 5) Or (TimeAuto > 5 And (CuentaGRande * 2) >= TimeAuto) Or Trim(Command) = "/01PEEKTOT" Or ((CuentaGRande * 2) >= TimeAutoSeg And (TimeAutoSeg <> 0)) Then
                ...
                If Not (lNoCoger) Or Dir(sPath & "reco.txt") <> "" Then
                    ...
                    If TipoTQT(0) = "1" Or TipoTQT(1) = "1" Then LeeTickets 0
                    'If Dir(App.Path & "\nolabel.txt") = "" Then
                        If TipoTQT(2) = "1" Then LeeTickets 1
                        'If Dir(App.Path & "\nopack.txt") = "" Then
                            If TipoTQT(3) = "1" Then LeeTickets 2
                        'End If
                    'End If
                ...
                Else
                    ...
                    Else
                        If Dir(App.Path & "\codisys.txt") <> "" Or lTqRetail Or Dir(App.Path & "\libravahy.txt") <> "" Or Dir(App.Path & "\ahorramas.cfg") <> "" Then
                ...
                If (Time >= dHoraFin And (dHoraFin <> CDate("00:00:00"))) And Dir(App.Path & "\" & Format(Date, "ddmmyy") & ".fin") = "" Then
                    ...
                    If PathSC10 <> "NOEXPORTAR" Then
                        ...
                        If lUpper Then
                            Call export_CABLIN(True)
                        Else
                            If Dir(App.Path & "\codisys.txt") <> "" Or lTqRetail Then
                                If nMaqType = 0 Then
                                    Principal.Taquion_EnviaTiquetsPendientes_ah True 'euroscale
                                Else
                                    Principal.Taquion_EnviaTiquetsPendientes_ah False 'sc10
                    ...
                    If Dir(App.Path & "\end_day.ini") <> "" Then
                        ...
                        MiProceso = OpenProcess(PROCESS_QUERY_INFORMATION, False, Shell("hydratouch /01END_DAY1", vbMinimizedFocus))
                ...
                Else
                    ...
                    If Dir(App.Path & "\ahorramas.cfg") = "" Then
                      ...
                      If (Dir(sPath & "global.dat") <> "" Or Dir(App.Path & "\global.dat") <> "") And (lUpper = False Or lNacho = True) Then
                          ...
                          'End If
                          Sleep (3000)
                          If UCase(sPath) <> UCase(App.Path & "\") Then
                            CadenadeLog "Detectado: " & sPath & "global.dat"
                            Do_Events
                            nLG = FileLen(sPath & "global.dat")
                            Do_Events
                            ...
                            Loop
                            On Error Resume Next
                            'FileCopy sPath & "global.dat", App.Path & "\globaltouch.dat"
                            FileCopy sPath & "global.dat", App.Path & "\global.dat"
                          End If
                          
                          CadenadeLog "LLamada HydraTouch proceso: " & "global.dat"
                          MiResultado = STILL_ACTIVE
                          MiProceso = OpenProcess(PROCESS_QUERY_INFORMATION, False, Shell("hydratouch /01GLOBAL", vbMinimizedFocus))
                          Do While MiResultado = STILL_ACTIVE
                              GetExitCodeProcess MiProceso, MiResultado
                              Do_Events
                          Loop
                          CadenadeLog "Procesado: " & "global.dat" & " Por parte de HydraTouch."
                          If lSerial Then
                              '''''''''''''''''''''''''
                              reset
                              '''''''''''''''''''''''''
                          End If
                          CadenadeLog "Envia de Modificaciones desde HydraTouch..."
                          MiResultado = STILL_ACTIVE
                          MiProceso = OpenProcess(PROCESS_QUERY_INFORMATION, False, Shell("hydratouch /01TZ", vbMinimizedFocus))
                          Do While MiResultado = STILL_ACTIVE
                              GetExitCodeProcess MiProceso, MiResultado
                              Do_Events
                          Loop
                          If Dir(App.Path & "\envok.dat") <> "" Then
                              CadenadeLog "Modificaciones Correctamente Enviadas Por parte de HydraTouch."
                          Else
                              CadenadeLog "Error en envio Modificaciones Por parte de HydraTouch."
                          End If
                          'Sleep (1000)
                      ...
                      If (Dir(sPath & "globaltouch.dat") <> "" Or Dir(App.Path & "\globaltouch.dat") <> "") And lUpper = True Then
                          If Dir(App.Path & "\globaltouch.dat") <> "" Then
                            CadenadeLog "Detectado: " & App.Path & "\globaltouch.dat"
                          Else
                            CadenadeLog "Detectado: " & sPath & "globaltouch.dat"
                            Do_Events
                            nLG = FileLen(sPath & "globaltouch.dat")
                            Do_Events
                            Sleep (3000)
                            Do_Events
                            Do While nLG <> FileLen(sPath & "globaltouch.dat")
                                Do_Events
                                nLG = FileLen(sPath & "globaltouch.dat")
                                Sleep (3000)
                                Do_Events
                            Loop
                            On Error Resume Next
                            FileCopy sPath & "\globaltouch.dat", App.Path & "\globaltouch.dat"
                            On Error GoTo 0
                          End If
                          
                          CadenadeLog "LLamada HydraTouch proceso: " & "globaltouch.dat"
                          MiResultado = STILL_ACTIVE
                          MiProceso = OpenProcess(PROCESS_QUERY_INFORMATION, False, Shell("hydratouch /01GLOBAL", vbMinimizedFocus))
                          Do While MiResultado = STILL_ACTIVE
                              GetExitCodeProcess MiProceso, MiResultado
                              Do_Events
                          Loop
                          CadenadeLog "Procesado: " & "globaltouch.dat" & " Por parte de HydraTouch."
                          If lSerial Then
                              '''''''''''''''''''''''''
                              reset
                              '''''''''''''''''''''''''
                          ...
                          'Sleep (1000)
                          If Dir(sPath & "globaltouch.dat") <> "" Then
                              Kill sPath & "globaltouch.dat"
                          End If
                          'Sleep (1000)
                          If Dir(App.Path & "\globaltouch.dat") <> "" Then
                              Kill App.Path & "\globaltouch.dat"
                          End If
                      End If
                      ...
                      If Dir(sPath & "fichas.dat") <> "" Then
                          ...
                          MiResultado = STILL_ACTIVE
                          MiProceso = OpenProcess(PROCESS_QUERY_INFORMATION, False, Shell("hydratouch /VAC", vbMinimizedFocus))
                          Do While MiResultado = STILL_ACTIVE
                              GetExitCodeProcess MiProceso, MiResultado
                              Do_Events
                          Loop
                      ...
                      If Dir(sPath & "orden.dat") <> "" Or Dir(sPath & "marticulo1.daf") <> "" Then
                          On Error Resume Next
                          ...
                          End If
                          If Dir(sPath & "marticulo1.daf") <> "" Then
                              FileCopy sPath & "\marticulo1.daf", App.Path & "\marticulo1.daf"
                          End If
                          ...
                          End If
                          If Dir(sPath & "marticulo1.daf") <> "" Then
                              Kill sPath & "marticulo1.daf"
                          End If
                          If Dir(App.Path & "\marticulo1.daf") <> "" Then
                              Kill App.Path & "\marticulo1.daf"
                          End If
                      ...
                      '
                      '    MiResultado = STILL_ACTIVE
                      '    MiProceso = OpenProcess(PROCESS_QUERY_INFORMATION, False, Shell("hydratouch /01ADICIONALZ", vbMinimizedFocus))
                      '    Do While MiResultado = STILL_ACTIVE
                      '        GetExitCodeProcess MiProceso, MiResultado
                      '        Do_Events
                      '    Loop
        ...
        Else
            ...
            If Dir(App.Path & "\envmod.txt") <> "" Then 'And Dir(App.Path & "\hydrarun.txt") = "" Then
