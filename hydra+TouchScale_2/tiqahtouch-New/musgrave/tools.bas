...
Dim NombrePrograma As String
    ...
    If LCase(Que_Ejecuta) = "hydra" Then NombrePrograma = "OLDhydra.EXE"
    If LCase(Que_Ejecuta) = "mng" Then NombrePrograma = "OLDhydra.EXE"
    If LCase(Que_Ejecuta) = "iv4com" Then NombrePrograma = "OLDhydra.EXE"
    If Dir(App.Path & "\" & NombrePrograma) <> "" Then
        Arch = FreeFile()
        Open App.Path & "\auxiliar.tmp" For Output As #Arch
        Print #Arch, App.Path & "\dbase.mdb"
