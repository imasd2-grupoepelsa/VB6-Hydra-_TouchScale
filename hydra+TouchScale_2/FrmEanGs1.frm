...
Private Sub CambiarIdioma()
    Me.Caption = CargaCadena(725) & " (EanGS1)"
    Frame2.Caption = " IA's "
...
Private Sub Refresca_Datos()
    ...
    Case 0
        ...
        ("select * from gen_sam where baja=false and tipo='cbgs1' and destino=1 and numero=" & ElCodigo)
     
    Case 1
        Registro.OpenRecordset _
        ("select * from gen_sam where baja=false and tipo='cbgs1' and destino=2 and numero=" & CmbNumero.TexT)
...
Private Sub CmdBorrar_Click()
    ...
    End If
    If Baja_Barrasgs1(MiCodigo) = 0 Then
...
Private Function Alta_Barrasgs1(MiCabecera As dB_EanGs1) As Integer
...
Dim numero As Integer
    ...
    If MiCabecera.Equipo = 0 Then
        Set myRs = myB.OpenRecordset("select * from gen_sam where tipo='cbgs1' and destino=1 and numero=" & CStr(MiCabecera.Mostrador) & " and baja=false")
        Dest = 1
        numero = MiCabecera.Mostrador
    Else
        Set myRs = myB.OpenRecordset("select * from gen_sam where tipo='cbgs1' and destino=2 and numero=" & CStr(MiCabecera.Equipo) & " and baja=false")
        Dest = 2
...
'Baja_Cabeceras
Private Function Baja_Barrasgs1(MiCabecera As dB_EanGs1) As Integer
Dim myRs As dao.Recordset
Dim myB As dao.Database
Dim Resp As Integer
Dim Dest As Integer
