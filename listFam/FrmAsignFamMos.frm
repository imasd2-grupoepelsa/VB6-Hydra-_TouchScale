...
'Dim lvwItem As Variant
    ...
    nC = 0
    ListFam.ListItems.Clear
    Set Registro = MyBase.OpenRecordset("select distinct(codi_fam),imagen,txt_fam from fam_code order by codi_fam")
    With Registro
        If Not .EOF Then
            .MoveFirst
            Do Until .EOF
                'Set lvwItem =
                ListFam.ListItems.Add , , Registro.Fields("txt_fam"), App.Path & "\vacio.jpg"
                .Movenext
                nC = nC + 1
            Loop
        End If
