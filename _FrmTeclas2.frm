...
Begin VB.Form FrmTeclas2 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Gestión de Teclas"
   ClientHeight    =   6570
   ClientLeft      =   5205
   ClientTop       =   2085
   ClientWidth     =   9990
   Icon            =   "FrmTeclas2.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6570
   ...
   Begin VB.Frame Frame2 
      Appearance      =   0  'Flat
      Caption         =   "Teclas por Mostrador o Equipo"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
      ...
      Begin VB.ComboBox CmbNumero 
         Height          =   288
         ItemData        =   "FrmTeclas2.frx":000C
         Left            =   1200
         List            =   "FrmTeclas2.frx":000E
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox CmbJuego 
         Height          =   288
         ItemData        =   "FrmTeclas2.frx":0010
         Left            =   3240
         List            =   "FrmTeclas2.frx":0032
         Style           =   2  'Dropdown List
         TabIndex        =   4
         Top             =   720
         Width           =   1092
      ...
      Begin VB.ComboBox CmbTeclas 
         Height          =   288
         ItemData        =   "FrmTeclas2.frx":0054
         Left            =   3240
         List            =   "FrmTeclas2.frx":006A
         Style           =   2  'Dropdown List
         TabIndex        =   1
         Top             =   360
         Width           =   1092
...
Private Sub CambiarIdioma()
    ...
    CmdParte(0).Caption = CargaCadena(712) & " 1"
    CmdParte(1).Caption = CargaCadena(712) & " 2"
    CmdParte(2).Caption = CargaCadena(712) & " 3"
    CmdParte(3).Caption = CargaCadena(712) & " 4"
    Command3.Caption = CargaCadena(7)
    Frame1.Caption = CargaCadena(713)
    Frame2.Caption = CargaCadena(714)
    FrmTeclas2.Caption = CargaCadena(22)
...
Private Sub Refresca_Datos()
    ...
    Else
        If UsaECO2 And lMarteNew Then '2.0.9 Dir(App.Path & "\marte.new") <> "" Then
            'c2f h+2.0.7 v4 alfa... se habilita juegos... de momento solo
            '                       tendrán efecto 0 y 1 y 1 solo para v4 alfa...
    ...
    If OptTipo(1).Value = True Then
        If lss165 Then
            ...
            Registro.OpenRecordset _
                     ("select * from teclas where borrado=false and numero_eqp=" & CmbNumero.TexT & " and tabla=0")
            
            Else
            Registro.OpenRecordset _
                     ("select * from teclas where borrado=false and numero_eqp=" & CmbNumero.TexT & " and tabla=1")
            End If
        Else
            Registro.OpenRecordset _
            ("select * from teclas where borrado=false and tabla=" & CmbJuego.TexT & "  and numero_eqp=" & CmbNumero.TexT)
        End If
        With Registro
            If Not .EOF Then
    ...
    Else
        ...
        & " and codi_ident=" & LaSeccion.codigo(1))
        With Registro
            If Not .EOF Then
                .MoveFirst
                Do Until .EOF
...
Private Sub CmbNumero_Click()
    ...
    If OptTipo(1).Value And IsNumeric(CmbNumero.TexT) Then
        Registro.OpenRecordset "select codi_ident from equipos where borrado=false and numero_eqp=" & CmbNumero.TexT & " and (modelo=0 or modelo=1)"
        If Not Registro.EOF Then CmdConsulta.Visible = True
    End If
...
Private Sub Form_Load()
    ...
    For Bucle = 0 To 99
        ...
        Text1(Bucle).TexT = ""
    Next Bucle
    CmbTeclas.TexT = CmbTeclas.List(1)
    ...
    End If
    On Error GoTo 0
    If Not UsaGamaAlta And TipoC9 Then
        OptTipo(0).Value = True
        'OptTipo(1).Enabled = False
    End If
    
    If UsaECO2 And lMarteNew Then '2.0.9 Dir(App.Path & "\marte.new") <> "" Then
        CmbTeclas.ListIndex = 0
        CmbTeclas.Enabled = False
        CmbJuego.ListIndex = 0
        CmbJuego.Enabled = False
        OptTipo(0).Value = True
...
Private Sub OptTipo_Click(Index As Integer)
    ...
    End If
    If OptTipo(0).Value = True Then
        LblMostrador.Caption = CargaCadena(302)  '"Mostrador"
        If Dir(App.Path & "\ikea.exe") <> "" Then
            CmdConsulta.Visible = True
        Else
            CmdConsulta.Visible = False
        End If
    Else
        LblMostrador.Caption = CargaCadena(303)  '"Equipo "
