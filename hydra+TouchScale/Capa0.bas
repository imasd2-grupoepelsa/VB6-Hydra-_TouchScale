...
' 2--> Configurado RS-232
' 3--> Configurado Path de Red
...
' Interna
Public TipoNumeracion As Integer
'******************************
' Buffer de envío
...
Public BufferRecMulti(MAX_MULTI_SOCKETS) As String
...
Public TCP_SENT_MULTI(MAX_MULTI_SOCKETS) As Boolean
Public TCP_REC_MULTI(MAX_MULTI_SOCKETS) As Boolean
'SQLSERVER..............................
...
''''''''''''''''''''''''''''''''''''''''
Public Sub Capa0_Reset_ScalePos()
...
' 0 --> OK, configurado
' 3 --> TCP/IP incorrecto
Public Function Capa0_Configure_ScalePos(ByVal MachineID As Long, ByVal TypeConf As Long, sConfigure As String) As Long
...
Dim Puerto As Long
    ...
    If bytes(0) <> "" And bytes(1) <> "" And bytes(2) <> "" And bytes(3) <> "" Then
        ...
        If Val(bytes(0)) < 256 And Val(bytes(1)) < 256 And Val(bytes(2)) < 256 And Val(bytes(3)) < 256 Then
            ...
            If Puerto >= 1 And Puerto <= 65535 Then
                sAddIp = CStr(bytes(0)) & "." & CStr(bytes(1)) & "." & CStr(bytes(2)) & "." & CStr(bytes(3)) & "," & CStr(Puerto)
                Resp = True
            End If
        Else
            Resp = False
    ...
    If sAddIp <> "" And Resp = True Then
        On Error Resume Next
        Resp = False
        CadenadeLog "Cadena de conexión(Uid y Pass no reales): Provider=SQLOLEDB;Uid=X;password=Y;Initial Catalog=ScalePos;Data Source=" & sAddIp
        MyConnObj.Open "Provider=SQLOLEDB;Uid=sa;password=masterkey;Initial Catalog=ScalePos;Data Source=" & sAddIp
        If Err.Number <> 0 Then
    ...
    End If
    If Resp Then
        Capa0_Configure_ScalePos = 0
        CadenadeLog "Conexión realizada con:" & sAddIp
    Else
        Capa0_Configure_ScalePos = 3
    End If
End Function
...
Public Function Capa0_Configure(ByVal MachineID As Long, ByVal TypeConf As Long, sConfigure As String) As Long
    ...
    If MachineID = 10 Then
        MyAux = sConfigure
        Capa0_Configure = Capa0_Configure_ScalePos(10, 0, MyAux)
    End If
