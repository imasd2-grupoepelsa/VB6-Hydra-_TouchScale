...
Public Function Alta_Articulo(Balenv As Boolean, Base As Database, RegArticulo As Recordset, Optional desc As Integer) As Integer
    ...
    If SUPECO And ((articulo.plu >= 2000) And (articulo.plu <= 2999)) Then
        ...
        GoTo fin
    End If
    If (var.C9Cod <> 0) And (articulo.plu > 6999) Then    '///cas.v118
        Retorno = 14
        GoTo fin
    End If
    ...
    If articulo.sec = 0 Then
        ...
        End If
    End If
    If (var.C9Cod <> 0) And (articulo.sec > 1) Then   '///cas.v118
        Retorno = 15
        GoTo fin
    End If
...
Public Function alta_txt2040(abase As Database, aart As registroarticulos) As Integer
    ...
    sSQL = "SELECT *" & _
           " FROM linTxt2040" & _
           " WHERE codigo=" & CStr(aart.cod) & _
           " AND plu=" & CStr(aart.plu) & _
           " AND mostrador=" & CStr(aart.sec) & _
           " ORDER BY CODIGO"
...
ret:
    rst.Close
    Set rst = Nothing
    If Err.Number <> 0 Then alta_txt2040 = 1
    Err.Clear
    On Error GoTo 0
End Function
''''''''''''
