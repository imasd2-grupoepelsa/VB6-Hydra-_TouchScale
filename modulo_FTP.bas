...
Public Function Trae_FTP(ByVal sFich As String) As Long
    ...
    Do While nInt < 3
        ...
        For Ret = 1 To 5
            Do_Events
        Next Ret
        'nLen = getfilesize(sFich, 0)
        'Do While nLen <> nLenOrg And frmControl.tmrFtp.Enabled
        '    nLen = getfilesize(sFich, 0)
        '    For Ret = 1 To 50
        '        Do_Events
        '    Next Ret
        '    nLenOrg = getfilesize(sFich, 0)
        'Loop
        If Not (frmControl.tmrFtp.Enabled) Then
            nInt = 3
            Exit Do
        End If
        Ret = Recibir_Fichero_FTP(sFich, App.Path & "\" & sFich, 0)
        If Ret = 1 Then
            If FileLen(App.Path & "\" & sFich) <> nLenOrg Then
                Kill App.Path & "\" & sFich
                nInt = 3
            End If
            Exit Do
    ...
    If nInt < 3 Then
        If Ret = 1 Then
            Ret = Borrar_Fichero_FTP(sFich, 0)
        End If
        Trae_FTP = 1
    Else
        Trae_FTP = 0
